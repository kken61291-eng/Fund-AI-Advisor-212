...
class TechnicalAnalyzer:
    ...
    def calculate_indicators(data_dict):
        try:
            ...
            return {
                "price": current_price, "quant_score": int(max(0, min(100, score))),
                "rsi": round(rsi, 2), "bias_20": round(bias_20, 2),
                "trend_weekly": trend_weekly, "atr": round(atr, 3),
                "macd": {"diff": round(macd_diff, 3), "trend": macd_trend},
                "kdj": {"j": round(j_val, 2)}, "flow": {"obv_slope": round(obv_slope, 2)},
                "quant_reasons": [] # 关键
            }
